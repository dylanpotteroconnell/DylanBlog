...
```{r}
weekly.tb <- weekly.tb %>%
  dplyr::select(start_date, gross, log_gross, releases, everything())
```
...
```{r}
weekly.tb %>%
  ggplot(aes(x = start_date, y = gross_ia_week_adj)) + 
  geom_line() + 
  xlab("Year") + 
  ylab("Gross ($)") + 
  ggtitle("Weekly (American) Box Office Gross")
...
```{r}
ss.season <- AddSeasonal(list(), weekly.tb$log_gross_ia, 
                         nseasons = 52)
ss.season <- AddStaticIntercept(ss.season, weekly.tb$log_gross_ia)
model.season <- bsts(weekly.tb$log_gross_ia, 
                     state.specification = ss.season,
                     niter = 500, 
                     ping = 0, 
                     seed = 21)

ss.llt <- AddSeasonal(list(), weekly.tb$log_gross_ia, 
                         nseasons = 52)
ss.llt <- AddLocalLinearTrend(ss.llt,
...
# Take in a model, and return the one step prediction matrix 
ComputeOneStepPreds <- function(model, dat, response_var) {
...
```{r}
tb.llt <- ComputeOneStepPreds(model.llt, weekly.tb, log_gross_ia)

PlotOneStepFitted(tb.llt)

tb.season <- ComputeOneStepPreds(model.season, weekly.tb, log_gross_ia)
...
os.preds.r.diff2 <- ComputeOneStepPreds(model.rdiff2, 
