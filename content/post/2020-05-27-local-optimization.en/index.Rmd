...
Three economists saw the opportunity for a natural experiment: only certain commuters were disrupted by the closure, and using the subway's tracking data, their movements could be compared to the unaffected group. And surprisingly, after the strike ended, life *didn't* quite go back to normal. Instead, some of the disrupted group stuck with their newly discovered routes, and their commuting time decreased as a result.
...
In this blog post, we will explore how this story (and its surprising outcome) can be naturally understood using the field of mathematical optimization. In fact, there are deep parallels between this theoretical framework, and the problems we confront in our daily lives. This proves useful in both directions---the intuition we have built from lived experience helps us learn the mathematical theory, while that mathematical theory can in turn identify the limitations of our intuition. 

We will use a trio of real world examples (this subway strike, the quest for the perfect cookie, and the search for the top of a mountain) as a lens through which we can understand the building blocks of optimization. Then, we will explore how the counterintuitive but common inclusion of algorithmic random noise provides insight, not just into these case studies, but into any attempt to improve one's life. 
